...
function getAllCategory() {
    return index_1.SystemClasses.filter((clsx, index) => index_1.SystemClasses.findIndex((e) => e.categoryId === clsx.categoryId) === index);
}
...
function getCategoryPartialMatch(query) {
    const _query = query
        ...
        .map((e) => e.toLowerCase())
        .filter((e) => !/[\s]/.test(e) && e !== "");
    return getAllCategory().filter((e) => _query.every((s) => `${e.categoryId.toLowerCase()} ${e.categoryName.toLowerCase()} ${e.categoryDescription.toLowerCase()} ${e.keyword.toLowerCase()}`.indexOf(s) !== -1));
}
...
function getMethodPartialMatch(query, categoryId) {
    const _query = query
        ...
        .map((e) => e.toLowerCase())
        .filter((e) => !/[\s]/.test(e) && e !== "");
    return getAllMethods(categoryId).filter((e) => _query.every((s) => `${e.methodId.toLowerCase()} ${e.methodName.toLowerCase()} ${e.methodDescription.toLowerCase()} ${e.keyword.toLowerCase()}`.indexOf(s) !== -1));
}
